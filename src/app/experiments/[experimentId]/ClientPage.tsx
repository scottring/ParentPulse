...
// Main page
// ================================================================
export default function GrowthItemWorkspace({ params }: { params: Promise<{ experimentId: string }> }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await getDoc(doc(firestore, 'growth_items', experimentId));
        if (cancelled) return;
        if (snap.exists()) {
          ...
          console.warn(`growth_item ${experimentId} not found`);
        }
      } catch (err) {
        ...
        if (code === 'permission-denied') {
          console.warn(`growth_item ${experimentId} not accessible (likely deleted)`);
        } else {
  ...
  }, [experimentId, user, authLoading, router]);
    ...
    <div className="press-binder" style={{ maxWidth: 952 }}>
      ...
      {(prevItem || nextItem) && (
        ...
        >
          {prevItem ? (
            <button
              onClick={() => router.push(`/experiments/${prevItem.growthItemId}`)}
              className="press-link-sm"
              style={{
                background: 'transparent',
                cursor: 'pointer',
          ...
          {nextItem ? (
            <button
              onClick={() => router.push(`/experiments/${nextItem.growthItemId}`)}
              className="press-link-sm"
              style={{
                background: 'transparent',
                cursor: 'pointer',
