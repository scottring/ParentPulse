...
exports.chatWithCoach = onCall(
    ...
    async (request) => {
      ...
      try {
        ...
        // Call Claude with context, grounded in who is speaking.
        const speaker = {
          userId: request.auth.uid,
          name: userData.name ||
            context.userNamesById[request.auth.uid] || "A family member",
          relationshipType: userData.role || null,
        };
...
exports.chatWithEntry = onCall(
    ...
    async (request) => {
      ...
      // Prior turns, carrying authorId so a multi-person thread can be
      // attributed turn-by-turn (was previously dropped).
      const priorMessages = [];
      ...
      // Retrieve RAG context (also resolves family display names).
      const context = await retrieveChatContext(
          familyId, message, effectivePersonIds, request.auth.uid,
      );

      // Who is speaking right now. This is what was missing: the entry's
      // author and the current chatter are NOT necessarily the same person
      // (a shared entry can be discussed by either partner).
      const speakerName =
        (userData && userData.name) ||
        context.userNamesById[request.auth.uid] ||
        "A family member";
      const speaker = {
        userId: request.auth.uid,
        name: speakerName,
        relationshipType: (userData && userData.role) || null,
      };
      const isSpeakerAuthor = entryData.authorId === request.auth.uid;
      const entryAuthorName = isSpeakerAuthor ?
        speakerName :
        (context.userNamesById[entryData.authorId] || "my partner");

      // Build messages for Claude. First turn carries the entry text,
      // phrased to reflect who ACTUALLY wrote it. Later turns attribute
      // each human turn by name when more than one person has spoken.
      const isFirstMessage = priorMessages.length === 0;
      let messagesForClaude;
      if (isFirstMessage) {
        messagesForClaude = [{
          role: "user",
          content: buildEntryFirstMessage({
        ...
        messagesForClaude = buildThreadMessages({
          priorTurns: priorMessages,
          newMessage: message,
          speakerId: request.auth.uid,
          speakerName,
          authorNamesById: context.userNamesById,
        });
      }
...
async function retrieveChatContext(familyId, userMessage, personIdOrIds = null, userId = null) {
  ...
  // Resolve display names for everyone in the family so journal entries
  // can be attributed to their author. Without this the model sees
  // author-less text blobs and can't tell two partners' entries apart.
  const familyUsersSnapshot = await admin.firestore()
      .collection("users")
      .where("familyId", "==", familyId)
      .get();
  const userNamesById = {};
  familyUsersSnapshot.forEach((doc) => {
    const d = doc.data();
    if (d && d.name) userNamesById[doc.id] = d.name;
  });
  const nameForAuthor = (authorId) => userNamesById[authorId] || "A family member";

  // Get recent journal entries (last 30 days). Scan a wider window than
  // we display so topic-linking can reach entries outside the recent 10.
  ...
  // Visibility filter, shared by recent + related views.
  const canSee = (d) => {
  ...
  const visibleDocs = journalSnapshot.docs.filter((doc) => canSee(doc.data()));
  ...
  const toEntry = (doc) => {
    ...
    return {
      ...
      authorId: d.authorId || null,
      authorName: nameForAuthor(d.authorId),
      personMentions: Array.isArray(d.personMentions) ? d.personMentions : [],
    };
  };

  const allVisibleEntries = visibleDocs.map(toEntry);
  // Recent view: the freshest 20, as before (but now author-attributed).
  const journalEntries = allVisibleEntries.slice(0, 20);

  // Related view: entries (from EITHER partner) that mention the same
  // person(s) this conversation is grounded in. This makes cross-partner
  // context deterministic instead of luck-of-the-recent-window — e.g.
  // both parents' entries about the same child surface together.
  let relatedEntries = [];
  if (personIds.length > 0) {
    const personSet = new Set(personIds);
    const recentIds = new Set(journalEntries.map((e) => e.id));
    relatedEntries = allVisibleEntries
        .filter((e) => e.personMentions.some((pid) => personSet.has(pid)))
        // Prefer entries not already shown in the recent list, but it's
        // fine for an entry to appear in both — the labels make it clear.
        .sort((a, b) => (recentIds.has(a.id) === recentIds.has(b.id) ? 0 : recentIds.has(a.id) ? 1 : -1))
        .slice(0, 8);
  }
  ...
  return {
    journalEntries,
    relatedEntries,    // entries about the same people (either partner)
    knowledgeItems,
    actions,
    ...
    workbooks,
    pastConversations,
    userNamesById,     // uid -> display name, for turn attribution
  };
}
...
async function generateChatResponse(messages, context, usageCtx = null) {
  ...
  // Build system message with context, grounded in who is speaking.
  let systemMessage = buildChatSystemMessage(
      context, usageCtx ? usageCtx.speaker || null : null);
